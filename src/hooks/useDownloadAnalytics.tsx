...
export const useDownloadAnalytics = () => {
  ...
  const { data: downloadStats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // If user is logged in, get their package analytics
      if (user?.id) {
        // Get user's packages
        ...
        // For logged out users, get global download statistics
        const { data: downloads, error: downloadsError } = await supabase
          .from('download_analytics')
          .select('*')
          .order('downloaded_at', { ascending: false });
    ...
    // Remove the enabled condition so it works for both logged in and logged out users
  });
